...
def from_start():
    ...
    start = time.time()
    rmgm_boost = RMGM_Boost(DATA_ROOT, SOURCE_RATING_FILE, TARGET_RATING_FILE, MINIMUM_X_CATEGORIES_FILENAME, folds=10)
...
def from_middle_nonRandomUsers():
    with open(os.path.join('C:\\RS\\Amazon\\Tresholds\\MINIMAL_THRESHOLD_30\\170816210819', 'rmgm_boost_save.pickle'), 'rb') as handle:
        rmgm_boost2 = pickle.load(handle)
        # rmgm_boost2.generate_mini_domains()
        rmgm_boost2.generate_folds()
